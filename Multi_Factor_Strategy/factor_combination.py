...
class FactorCombiner:
    ...
    def model1_factor_selection_equal_weight(self, 
        ...
        signal_path : str, 信号保存路径，如果提供且文件已存在则跳过计算
        ...
        if signal_path is not None:
            import os
            signal_file = os.path.join(signal_path, 'model1_factor_selection_prediction.csv')
            if os.path.exists(signal_file):
                print(f"⏭️  检测到已存在的模型1信号文件: {signal_file}")
                print("   直接加载，跳过计算...")
                try:
                    prediction_df = pd.read_csv(signal_file, index_col=0, encoding='utf-8-sig')
                    # 转换列名为日期格式
                    try:
                        prediction_df.columns = pd.to_datetime(prediction_df.columns)
                    except:
                        pass
                    self.model1_prediction = prediction_df
                    print(f"✅ 模型1信号加载完成，形状: {prediction_df.shape}")
        ...
        print(f"  测试集日期: {len(test_dates)} 个 ({test_dates[0]} 至 {test_dates[-1]})")
        print(f"  将对所有日期进行日频预测...")
        ...
        # 对所有日期进行预测（向量化优化）
        date_iterator = tqdm(dates, desc="等权合成（日频）") if TQDM_AVAILABLE else dates
        ...
        for date in date_iterator:
            ...
            # 过滤有效样本（允许部分因子缺失，使用截面均值填充）
            min_valid_ratio = 0.3
            min_valid_count = int(len(factor_values_list) * min_valid_ratio)
            
            # 计算每个股票的因子有效数量
            valid_factor_counts = (~np.isnan(factor_matrix)).sum(axis=1)
            valid_mask = valid_factor_counts >= min_valid_count
            
            if valid_mask.sum() == 0:
                continue
            
            factor_matrix_valid = factor_matrix[valid_mask].copy()
            valid_stocks = [common_stocks[i] for i in range(len(common_stocks)) if valid_mask[i]]
            
            # 对缺失值进行填充（使用截面均值）
            for j in range(factor_matrix_valid.shape[1]):
                col_values = factor_matrix_valid[:, j]
                col_mean = np.nanmean(col_values)
                if np.isnan(col_mean):
                    col_mean = 0.0
                col_values[np.isnan(col_values)] = col_mean
                factor_matrix_valid[:, j] = col_values
    ...
    def model2_mlp(self, train_ratio=0.6, val_ratio=0.2, test_ratio=0.2,
        ...
        if signal_path is not None:
            import os
            signal_file = os.path.join(signal_path, 'model2_mlp_prediction.csv')
            if os.path.exists(signal_file):
                print(f"⏭️  检测到已存在的模型2信号文件: {signal_file}")
                print("   直接加载，跳过计算...")
                try:
                    prediction_df = pd.read_csv(signal_file, index_col=0, encoding='utf-8-sig')
                    # 转换列名为日期格式
                    ...
                    print(f"✅ 模型2信号加载完成，形状: {prediction_df.shape}")
                    return prediction_df
                except Exception as e:
                    print(f"⚠️  加载失败，将重新计算: {e}")
        ...
        predictions_test = scaler_y.inverse_transform(predictions_test.reshape(-1, 1)).flatten()
        
        # 预测训练集和验证集（用于完整性）
        predictions_train_val = []
        train_val_dataset = FactorDataset(
        ...
        with torch.no_grad():
            for batch_X, _ in train_val_loader:
                batch_X = batch_X.to(DEVICE)
                outputs = model(batch_X)
                predictions_train_val.append(outputs.cpu().numpy())
        
        predictions_train_val = np.concatenate(predictions_train_val)
        predictions_train_val = scaler_y.inverse_transform(predictions_train_val.reshape(-1, 1)).flatten()
        ...
        # 保存训练集和验证集预测
        stock_info_train_val = stock_info[:n_train+n_val]
        for i, info in enumerate(stock_info_train_val):
            if i < len(predictions_train_val) and not np.isnan(predictions_train_val[i]):
                stock = info['stock']
                date = info['date']
                if stock in prediction_df.index and date in prediction_df.columns:
                    prediction_df.loc[stock, date] = predictions_train_val[i]
        ...
        # 对所有日期进行预测（日频预测）
        print("\n对所有日期进行日频预测...")
        all_dates = sorted(self.return_data.columns)
        all_predictions = self._predict_all_dates_mlp(model, scaler_X, scaler_y, all_dates, stocks)
        
        # 更新预测值：如果某日期已有预测值（来自训练/验证/测试集），保留；否则使用全量预测
        for date in all_dates:
            for stock in stocks:
                if pd.isna(prediction_df.loc[stock, date]):
                    if stock in all_predictions.index and date in all_predictions.columns:
                        prediction_df.loc[stock, date] = all_predictions.loc[stock, date]
    ...
    def model3_xgboost(self, train_ratio=0.8, test_ratio=0.2,
                      ...
                      subsample=0.8, colsample_bytree=0.8, signal_path=None):
        """
        模型3：XGBoost模型
        ...
        if signal_path is not None:
            import os
            signal_file = os.path.join(signal_path, 'model3_xgboost_prediction.csv')
            if os.path.exists(signal_file):
                print(f"⏭️  检测到已存在的模型3信号文件: {signal_file}")
                print("   直接加载，跳过计算...")
                try:
                    prediction_df = pd.read_csv(signal_file, index_col=0, encoding='utf-8-sig')
                    # 转换列名为日期格式
                    ...
                    print(f"✅ 模型3信号加载完成，形状: {prediction_df.shape}")
                    return prediction_df
                except Exception as e:
                    print(f"⚠️  加载失败，将重新计算: {e}")
        ...
        # 创建XGBoost模型（添加正则化防止过拟合）
        print("\n开始训练XGBoost模型（使用验证集早停防止过拟合）...")
        
        # 使用合适的tree_method（XGBoost不支持'gpu_hist'，使用'hist'或'auto'）
        # 'auto'会自动选择最佳方法，'hist'是直方图方法（更快且兼容性好）
        tree_method = 'hist'  # 使用hist方法，兼容性更好
        
        # 进一步划分训练集：80%用于训练，20%用于验证（用于早停）
        n_train_final = int(len(X_train) * 0.8)
        X_train_final = X_train_scaled[:n_train_final]
        y_train_final = y_train[:n_train_final]
        X_val_final = X_train_scaled[n_train_final:]
        y_val_final = y_train[n_train_final:]
        
        # 使用验证集进行早停（防止过拟合）
        # XGBoost 2.0+使用callbacks（在模型初始化时设置），旧版本使用early_stopping_rounds（在fit方法中）
        use_early_stopping = True
        best_iteration = n_estimators
        
        try:
            # 尝试新版本方式：在模型初始化时设置callbacks
            ...
            model = xgb.XGBRegressor(
                ...
                callbacks=[early_stop]  # 在初始化时设置callbacks
            )
            ...
            best_iteration = model.best_iteration if hasattr(model, 'best_iteration') and model.best_iteration is not None else n_estimators
            
        except (ImportError, AttributeError, TypeError, ValueError):
            # 如果新版本方式失败，尝试旧版本方式：在fit方法中使用early_stopping_rounds
            try:
                model = xgb.XGBRegressor(
                    n_estimators=n_estimators,
                    max_depth=max_depth,
                ...
                # 如果都不支持，不使用早停，直接训练
                print("  ⚠️  当前XGBoost版本不支持早停，将使用完整训练")
        ...
        # 如果使用了早停且找到了最佳迭代次数，使用完整训练集重新训练
        if use_early_stopping and best_iteration < n_estimators:
            print(f"  早停于第 {best_iteration} 轮迭代（共 {n_estimators} 轮）")
            # 使用最佳迭代次数重新训练完整训练集
            model = xgb.XGBRegressor(
        ...
        print("\n对所有日期进行日频预测...")
        all_dates = sorted(self.return_data.columns)
        all_predictions = self._predict_all_dates(model, scaler_X, all_dates, stocks)
        
        # 更新预测值：如果某日期已有预测值（来自训练/测试集），保留；否则使用全量预测
        for date in all_dates:
            for stock in stocks:
                if pd.isna(prediction_df.loc[stock, date]):
                    if stock in all_predictions.index and date in all_predictions.columns:
    ...
    def _predict_all_dates(self, model, scaler_X, dates, stocks):
        """
        对所有日期进行预测（XGBoost）
        ...
        # 批量处理日期
        iterator = tqdm(dates, desc="日频预测") if TQDM_AVAILABLE else dates
        ...
        for date in iterator:
            ...
            for factor_name in all_factors:
                if factor_name in factor_values_dict:
                    factor_series = factor_values_dict[factor_name]
                    common_values = factor_series.loc[common_stocks]
                    valid_values = common_values.dropna()
                    if len(valid_values) > 0:
                        factor_means[factor_name] = valid_values.mean()
                    else:
            ...
            # 构建特征矩阵
            X_date = np.full((len(common_stocks), len(all_factors)), np.nan)
            for idx, stock in enumerate(common_stocks):
                for j, factor_name in enumerate(all_factors):
                    if factor_name in factor_values_dict:
                        value = factor_values_dict[factor_name].loc[stock]
                        if pd.isna(value):
                            X_date[idx, j] = factor_means[factor_name]
                        else:
                            X_date[idx, j] = value
                    else:
                        X_date[idx, j] = 0.0
            ...
            # 标准化并预测
            X_date_scaled = scaler_X.transform(X_date)
            predictions_date = model.predict(X_date_scaled)
            
            # 保存预测值
            for idx, stock in enumerate(common_stocks):
                if not np.isnan(predictions_date[idx]):
                    prediction_df.loc[stock, date] = predictions_date[idx]
        
        return prediction_df
    
    def _predict_all_dates_mlp(self, model, scaler_X, scaler_y, dates, stocks):
        """
        对所有日期进行预测（MLP）
        ...
        for date in iterator:
            ...
            # 标准化并预测
            X_date_scaled = scaler_X.transform(X_date)
            X_date_tensor = torch.FloatTensor(X_date_scaled).to(DEVICE)
            
            model.eval()
            with torch.no_grad():
                predictions_scaled = model(X_date_tensor).cpu().numpy()
            
            # 反标准化
            predictions_date = scaler_y.inverse_transform(predictions_scaled.reshape(-1, 1)).flatten()
...
def main(factors_path='./factors/', data_path='./data/', signal_path='./signal/'):
    ...
    model1_pred = combiner.model1_factor_selection_equal_weight(
        ...
        use_analysis_results=True,
        signal_path=signal_path  # 传入signal_path用于检查已存在的信号
    )
    ...
    # 检查是否已存在最终信号文件
    final_signal_file = os.path.join(signal_path, 'final_signal.csv')
    if os.path.exists(final_signal_file):
        print(f"⏭️  检测到已存在的最终信号文件: {final_signal_file}")
        print("   直接加载，跳过合成...")
        try:
            final_signal = pd.read_csv(final_signal_file, index_col=0, encoding='utf-8-sig')
            # 转换列名为日期格式
            try:
                final_signal.columns = pd.to_datetime(final_signal.columns)
            except:
                pass
            combiner.final_signal = final_signal
            print(f"✅ 最终信号加载完成，形状: {final_signal.shape}")
        except Exception as e:
            print(f"⚠️  加载失败，将重新合成: {e}")
